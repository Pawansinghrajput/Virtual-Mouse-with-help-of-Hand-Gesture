...
while True:
    ...
    if len(lmList) != 0:
        ...
        if fingers[1] == 1 and fingers[2] == 0:
            plocationX, plocationY, img = ops.move_mouse(x1, y1, cam_width, cam_height, screen_width, screen_height, frameR,  smoothening, plocationX, plocationY, img)
            
        # If the index and middle fingers are up, click the mouse
        if fingers[1] == 1 and fingers[2] == 1 and fingers[3] == 0 and fingers[4] == 0:
            img, click_status = ops.click_mouse(detector, img, click_status)
    ...
    fps = 1 / (cTime - pTime)
    pTime = cTime
    cv2.putText(img, "FPS: "+str(int(fps)), (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
    cv2.putText(img, "Raise pinky finger to exit", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
